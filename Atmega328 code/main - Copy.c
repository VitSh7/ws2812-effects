	...
	for (int i=0; i<FHT_N; i++)
	{
		...
		if (MONO==0)
		{
			...
			if(R_sound_level<max_sound_noise)
				R_sound_level=0;
			if(R_sound_level>L_current_level)
...
void level_filter()
{
	...
	{
		int average = (L_soundLevel+R_soundLevel)/2;
		L_soundLevel=average;
		R_soundLevel=average;
	}
